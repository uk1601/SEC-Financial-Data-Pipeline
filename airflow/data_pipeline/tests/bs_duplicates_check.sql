-- -----------------------------------------------------------------------------
-- File: airflow/data_pipeline/tests/bs_duplicates_check.sql
--
-- Test: Ensure no duplicate Balance Sheet records
-- Validates that the balance_sheet mart has no repeated rows for the same
...
FROM {{ ref('balance_sheet') }}  -- Reference to the balance_sheet mart model
GROUP BY
    COMPANY_NAME,
    COMPANY_ID,
    FILING_DATE,
